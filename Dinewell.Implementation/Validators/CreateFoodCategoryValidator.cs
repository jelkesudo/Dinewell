...
namespace Dinewell.Implementation.Validators
{
    ...
    {
        public CreateFoodCategoryValidator(DinewellContext context) 
        {
           ...
           .Must(x => !context.FoodCategories.Any(f => f.Name.ToLower() == x.ToLower())).WithMessage(x => $"Food Category {x.Name} already exists in the database.");
        }
    }
}
