...
{
    public class EfSearchMealsQuery : EfUseCase, ISearchMealsQuery
    {
        public EfSearchMealsQuery(DinewellContext context) : base(context)
        {
        }

        public int Id => 10;

        public string Name => "Search Meals (EF)";

        public string Description => "Search all of the meals in the app.";
        ...
        public PagedResponse<MealSeparateDTO> Execute(MealSearch search)
        {
            ...
            response.Data = query.Skip(toSkip).Take(search.PerPage.Value).Where(x => x.IsActive == true).Select(x => new MealSeparateDTO
            {
                Id = x.Id,
                Name = x.Food.Name,
