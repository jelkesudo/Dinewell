...
{
    public class EfUserSearchHisOrdersQuery : UseCases.EfUseCase, IUserSearchHisOrdersQuery
    {
        public EfUserSearchHisOrdersQuery(DinewellContext context) : base(context)
        {
        }

        public int Id => 4;

        public string Name => "User search his orders (Ef)";

        public string Description => "";

        public PagedResponse<OrderDTO> Execute(UserOrderSearch search)
        {
            var query = Context.Orders.Where(x => x.UserId == search.Id);
