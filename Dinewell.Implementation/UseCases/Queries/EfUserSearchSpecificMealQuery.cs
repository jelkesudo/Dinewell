...
{
    public class EfUserSearchSpecificMealQuery : UseCases.EfUseCase, IUserSearchSpecificMealQuery
    {
        public EfUserSearchSpecificMealQuery(DinewellContext context) : base(context)
        {
        }

        public int Id => 10;

        public string Name => "User search specific meals (EF)";

        public string Description => "";

        public MealSeparateDTO Execute(int search)
        {
            var query = Context.RestaurantMenus.FirstOrDefault(x => x.Id == search && x.IsActive);
